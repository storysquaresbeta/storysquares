...
class Projects_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if (!$client_id && $user_id) {
            ...
            //added by GS
            $classadmin_id = get_array_value($options, "classadmin_id");
            $project_query = get_array_value($options, "project_query");
            if($classadmin_id){
                $extra_where .= " AND project_members_table.project_id IN (".implode(',', $project_query).")";
                
                //$extra_join = " LEFT JOIN (SELECT $project_members_table.user_id, $project_members_table.project_id FROM $project_members_table WHERE ($project_members_table.user_id IN ($user_id, $classadmin_id)) AND $project_members_table.deleted=0 ) AS project_members_table ON project_members_table.project_id= $projects_table.id ";
              //  $extra_where = " AND (project_members_table.user_id=$user_id AND project_members_table.user_id=$classadmin_id)";
            }
        }
    ...
    function get_user_projects($user_id) {
        ...
        $sql = "SELECT $project_members_table.project_id
        FROM $project_members_table
        WHERE $project_members_table.deleted=0 AND user_id = $user_id";
        return $this->db->query($sql);
    }
    
    function get_project_clients($project_ids) {
        $projects_table = $this->db->dbprefix('projects');

        $sql = "SELECT $projects_table.client_id
        FROM $projects_table
        WHERE $projects_table.deleted=0 AND id IN (".implode(',', $project_ids).") group by $projects_table.client_id";
        return $this->db->query($sql);
    }
    ...
    function delete_project_and_sub_items($project_id) {
        $projects_table = $this->db->dbprefix('projects');
        $tasks_table = $this->db->dbprefix('tasks');
        $tasks_block_table = $this->db->dbprefix('task_blocks');
        $milestones_table = $this->db->dbprefix('milestones');
        ...
        //added by GS
        $delete_tasks_block_sql = "UPDATE $tasks_block_table SET $tasks_block_table.deleted=1 WHERE $tasks_block_table.project_id=$project_id; ";
        $this->db->query($delete_tasks_block_sql);
