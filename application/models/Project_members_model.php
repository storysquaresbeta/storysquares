...
class Project_members_model extends Crud_model {
    ...
    //check existing student in a project. Allow only a single student per project
    function get_students($options = array()){
        $list_data = $this->get_details($options);
        print_r($list_data);
    }
...
//Edited by GS
    //only allow single student per project
    function get_rest_team_members_for_a_project($project_id = 0) {
        ...
        FROM $roles_table
        WHERE $roles_table.role_type='student' AND $roles_table.deleted=0 ";
           $role =  $this->db->query($sql)->row();
          $role_id = $role->id;
          $sql = "SELECT $project_members_table.user_id FROM $project_members_table LEFT JOIN $users_table ON $project_members_table.user_id=$users_table.id WHERE $project_members_table.project_id='$project_id' AND $project_members_table.deleted=0 and $users_table.role_id = '$role_id'";
        ...
        if($count > 0){
            $sql .= " AND $users_table.role_id != '$role_id'";
        }
