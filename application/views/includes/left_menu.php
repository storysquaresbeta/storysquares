<div id="sidebar" class="box-content ani-width">
    <div id="sidebar-scroll">
        ...
        <ul class="" id="sidebar-menu">
            <?php //Added by GS ?>
            <li class="main">  
                <?php if ($this->login_user->role_type != 'student'){ ?>
             <a href="<?php echo_uri('dashboard'); ?>"><img src="<?php echo get_logo_url(); ?>" /></a>
                <?php }else{ ?>
             <a href="<?php echo_uri('projects/all_projects'); ?>"><img src="<?php echo get_logo_url(); ?>" /></a>
                <?php } ?>
            </li>
            <?php
...
print_r($data);exit;*/
            if ($this->login_user->user_type == "staff") {
                ...
                if ($this->login_user->role_type != 'student'){
                    $sidebar_menu[] = array("name" => "dashboard", "url" => "dashboard", "class" => "fa-desktop");
                }
                ...
                //Edited by GS - projects menu item moved to top.
                if($project_info){ 
                    $name = $project_info->title;
                    if ($this->login_user->role_type == 'student'){
                        $project_detail_menu = array(                        
                        array("name" => "view_story", "url" => "projects/tasks_kanban/" . $project_info->id, "attributes" => "data-target=\"#project-tasks-kanban-section\" "),
                            array("name" => "overview", "url" => "projects/overview/" . $project_info->id, "attributes" => "data-target=\"#project-overview-section\" "),

                        );

                        $sidebar_menu[] = array("name" => $name, "url" => "projects/view/" . $project_info->id, "checkactive" => "projects", "ajaxtab" => true, "class" => "fa-th-large",
                            "submenu" => $project_detail_menu
                        );
                    }else{
                        $project_detail_menu = array(
                        array("name" => "overview", "url" => "projects/overview/" . $project_info->id, "attributes" => "data-target=\"#project-overview-section\" "),
                    ...
                    $project_submenu = array(
                       ...
                       /* array("name" => "tasks", "url" => "projects/all_tasks")*/
                        );//Edited by GS - remove tasks link from sidebar
                ...
                if (get_setting("module_note") == "1") {
                    /*$sidebar_menu[] = array("name" => "notes", "url" => "notes", "class" => "fa-book font-16");*/ //Edited by GS - remove notes link from sidebar
                }
                ...
                if (get_array_value($this->login_user->permissions, "hide_team_members_list") != "1") {
                    if (!$this->login_user->is_admin) {
                        $sidebar_menu[] = array("name" => "team_students", "url" => "team_members", "class" => "fa-user font-16");
                    }else{
                ...
                if ($module_help || $module_knowledge_base) {
                  ...
                  $sidebar_menu[] = array("name" => "my_profile", "url" => "team_members/view/" . $this->login_user->id, "class" => "fa-cog");
            ...
            foreach ($sidebar_menu as $main_menu) {
                ...
                $expend_class_custom = $submenu ? " expand-sub " : "";
                if($main_menu['checkactive'] == 'projects')
                    $active_class = active_menu($main_menu['checkactive'], $submenu);
                else
                ...
                ?>
                <li class="<?php echo $active_class . " " . $expend_class_custom . " " . $submenu_open_class . " $devider_class"; ?> main">
                    <a href="<?php echo_uri($main_menu['url']); ?>" <?php echo ($main_menu['attributes']); ?>>
                        <i class="fa <?php echo ($main_menu['class']); ?>"></i>
                        <span><?php echo (lang($main_menu['name'])!= ""?lang($main_menu['name']):$main_menu['name']); ?></span>
    <?php
    if ($badge) {
    ...
    ?>
                        ...
                        <?php
                        if ($submenu) {
                            if($main_menu['ajaxtab'])
                                $ajaxtab = 'data-toggle="ajax-tab"';
                            else
                                $ajaxtab = '';
                            echo "<ul {$ajaxtab}>";
                            foreach ($submenu as $s_menu) {
                                ?>
                        <li>
                            <a href="<?php echo_uri($s_menu['url']); ?>" <?php echo ($s_menu['attributes']); ?>>
