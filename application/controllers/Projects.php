...
class Projects extends MY_Controller {
    ...
    //Added by GS
    function student_projects($user_id = -1) {
        $label_suggestions = array(array("id" => "", "text" => "- " . lang("label") . " -"));
        $labels = explode(",", $this->Projects_model->get_label_suggestions());
        $temp_labels = array();
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            $this->template->rander("projects/student_projects", $view_data);
        } else {
            $view_data['client_id'] = $this->login_user->client_id;
            $view_data['page_type'] = "full";
            $this->template->rander("clients/projects/index", $view_data);
    ...
    function file_download_prompt() {
        ...
        $this->load->view('projects/file_download', $view_data);
    }
...
//Edited by GS
     function save_cloned_project() {
        ...
        $task_blocks = $this->Task_blocks_model->get_all_where(array("project_id" => $project_id, "deleted" => 0))->result();

        $task_block_ids = array();
        
        foreach ($task_blocks as $task_block) {
            //prepare new task data. 
            $task_block->project_id = $new_project_id;
            
            $task_block->created_by = $this->login_user->id;
            $task_block->created_at = get_current_utc_time();            

            $task_block_data = (array) $task_block;
            unset($task_block_data["id"]); //remove id from existing data
            //add new task block
            
            $new_task_block_id = $this->Task_blocks_model->save($task_block_data);

            $task_block_ids[$task_block->id] = $new_task_block_id; //bind old id with new
        } 
        ...
        foreach ($tasks as $task) {
           ...
           // }
            ...
            $task->task_block_id = $task_block_ids[$task->task_block_id]; //bind old id with new

            $task_data = (array) $task;
            unset($task_data["id"]); //remove id from existing data
            unset($task_data["title"]); //remove title from existing data
            unset($task_data["description"]); //remove description from existing data
    ...
    function save_cloned_project_orig() {
    ...
    function student_projects_list($user_id = -1) {
        $this->access_only_team_members();
        ...
        $options["user_id"] = $user_id;//filter by users        
        $options["classadmin_id"] = $this->login_user->id;
        
        $result = array();
        
        if ($this->login_user->role_type === 'classadmin' || $this->login_user->is_admin){ 
            $projects = $this->Projects_model->get_user_projects($this->login_user->id)->result();
        
            $project_query = array();
            if(count($projects) > 0){
                foreach($projects as $project){
                    $project_query[] = $project->project_id;
                }
            }

            $options["project_query"] = $project_query;
...
//Edited by GS
    ...
    function project_member_modal_form() {
       ...
       // $this->Project_members_model->get_students(array('project_id' => $project_id));
        $view_data['project_id'] = $project_id;
    ...
    function all_tasks_kanban_data() {
        ...
        //Edited by GS
        $task_blocks = $this->Task_blocks_model->get_details(array("project_id" => $project_id));
         
        $view_data["total_columns"] = $task_blocks->num_rows();
        $view_data["columns"] = $task_blocks->result();

        $this->load->view('projects/tasks/kanban/kanban_view', $view_data);
    }

    function project_tasks_kanban_data_export($project_id = 0) {
        $this->init_project_permission_checker($project_id);
        ...
        foreach ($view_data["tasks"] as $task) {
            ...
            $item_list = $exising_items .  $task->description;
        ...
        //export to word file
        require_once(APPPATH . "libraries/phpword/bootstrap.php");
        ...
        // Add text elements
        $section->addTitle($view_data['project_info']->title, 0);
        $section->addTextBreak(1);

         foreach ($view_data["tasks"] as $column) {  
             $addtext = get_array_value($columns_list_data, $column->id); 
             $section->addText(
                $addtext
            );
        }
        
        $file = 'task_export.docx';
    ...
    function project_tasks_kanban_data_export_pdf($project_id = 0) {
        $this->init_project_permission_checker($project_id);
        ...
        //export to word file
        require_once(APPPATH . "libraries/wordwrap.php");
       
        foreach ($view_data["tasks"] as $column) {  
             $pdftext .= '<p>'.get_array_value($columns_list_data, $column->id).'</p>';            
        }
        ...
        $pdf->Cell(0,10,$view_data['project_info']->title,0,1,'C');
        $pdf->SetFont('Arial','',12);
        $pdf->WriteHTML($pdftext);
        $pdf->Output('D', 'task_export.pdf');
        exit;
    ...
    function task_modal_form() {
        ...
        if ($project_id) {
            $this->init_project_permission_checker($project_id);
//Edited by GS
            $related_data = $this->get_all_related_data_of_project($project_id, $view_data['model_info']->task_block_id);
            ...
            //Added by GS
            $view_data['task_blocks_dropdown'] = $related_data["task_blocks_dropdown"];
            ...
            //Added by GS
            $view_data['task_blocks_dropdown'] = array("" => "-");
        }
...
//Edited by GS
    private function get_all_related_data_of_project($project_id, $task_block_id = '') {
        ...
        if ($project_id) {
            ...
            if(count($tasks) > 0){
                foreach ($tasks as $key => $value) {
                    if($value->task_block_id && $value->task_block_id != $task_block_id)
                        $existing_blocks[] = $value->task_block_id;
                }
            }
           
            $task_blocks = $this->Task_blocks_model->get_dropdown_list(array("title"), "id", array("project_id" => $project_id));
           
            $task_blocks_dropdown = array("" => "-");
            /*foreach ($task_blocks as $key => $value) {
                if(!in_array($key, $existing_blocks))
                    $task_blocks_dropdown[] = array($key => $value);
            }*/
            foreach ($task_blocks as $key => $value) {
                if(!in_array($key, $existing_blocks))
                    $task_blocks_dropdown[$key] = $value;
            }
            ...
            return array(
                ...
                "task_blocks_dropdown" => $task_blocks_dropdown,//Added by GS
            );
        }
    }
    ...
    function get_all_related_data_of_selected_project($project_id) {
        ...
        if ($project_id) {
            ...
            echo json_encode(array(
                ...
                "task_blocks_dropdown" => $related_data["task_blocks_dropdown"],//added by GS
            ));
        }
    }
    ...
    function save_task() {
        ...
        $data = array(
            ...
            "project_id" => $project_id,
            "milestone_id" => $this->input->post('milestone_id'),
            "task_block_id" => $this->input->post('task_block_id'),//Added by GS
            "color" => $this->input->post('color'),//Added by GS
        ...
        if ($save_id) {
            ...
            if ($id) {
                ...
                //Added by GS
                //create new block and assign to task record
                //get sort value
               /* $max_sort_value = $this->Task_blocks_model->get_max_sort_value_by_project($project_id);
                $block_sort = $max_sort_value * 1 + 1; //increase sort value
                $block_data = array(
                    "project_id" => $project_id,
                    "title" => '',
                ...
                $block_save_id = $this->Task_blocks_model->save($block_data);
                //resave task
                $extra_data['task_block_id'] = $block_save_id;
                $this->Tasks_model->save($extra_data, $save_id);*/
                //created
    ...
    function save_task_sort_and_block() {
        $project_id = $this->input->post('project_id');
       // $this->init_project_permission_checker($project_id);
        ...
        $save_id = $this->Task_blocks_model->save($data, $status_id);

        if ($save_id) {
            if ($status_id) {
                $task_info = $this->Task_blocks_model->get_one($save_id);
                log_notification("project_task_updated", array("project_id" => $task_info->project_id, "task_block_id" => $save_id, "activity_log_id" => get_array_value($data, "activity_log_id")));
            }
        } else {
            echo json_encode(array("success" => false, lang('error_occurred')));
        }
