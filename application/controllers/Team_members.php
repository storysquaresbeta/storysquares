...
class Team_members extends MY_Controller {
    ...
    function modal_form() {
        ...
        if(get_array_value($this->login_user->permissions, "can_create_members") == "1")
        {
            //do not check if admin. Allo classroom admin to create member
        }else{
    ...
    function add_team_member() {
        ...
        if(get_array_value($this->login_user->permissions, "can_create_members") == "1")
        {
            //do not check if admin. Allow classroom admin to create member
        }else{
            $this->access_only_admin();
        }
        ...
        validate_submitted_data(array(
            ...
            "first_name" => "required",
            "last_name" => "required",
            //"job_title" => "required", //Edited by GS - job title not required
            "role" => "required"
        ));
        ...
        $user_data = array(
            ...
            "created_by" => $this->login_user->id //Added by GS
        );
        ...
        foreach ($roles as $record) {
            $role_dropdown[$record->id] = $record->title;
        }
        if($role == 'admin')
            $user_data['job_title'] = 'Admin';
        else
            $user_data['job_title'] = $role_dropdown[$role_id];
        $user_data['gender'] = '';
    ...
    function list_data() {
        ...
        if ($this->login_user->role_type === 'classadmin'){ 
            $options['role_type'] = 'classadmin';
        }
    ...
    private function _make_row($data, $custom_fields) {
        ...
        //edited by GS
        if ($this->login_user->role_type === 'classadmin'){ 
            $team_member_link = anchor('projects/student_projects/'.$data->id, $full_name, array());
        }else{
            $team_member_link = get_team_member_profile_link($data->id, $full_name);
        }
    ...
    function view($id = 0, $tab = "") {
        if ($id * 1) {
            ...
            if ($user_info) {
                ...
                $view_data['show_projects'] = false;//Added by GS - hide projects tab
    ...
    private function _get_roles_dropdown() {
        $role_dropdown = array(
           // "0" => lang('team_member'), //Edited by GS - hide team member group
